...
class TrafficUniTSEncoder(nn.Module):
    """Adapt UniTS blocks to the project's raw traffic sequence encoder interface."""
    ...
    def __init__(
        ...
        max_patch_tokens = max(1, calculate_unfold_output_length(max_len, patch_len, stride))
        self.position_embedding = LearnablePositionalEmbedding(embed_dim, max_len=max_patch_tokens + 1)
        self.cls_tokens = nn.Parameter(torch.zeros(1, input_dim, 1, embed_dim))
        torch.nn.init.normal_(self.cls_tokens, std=.02)
        mlp_ratio = max(float(expand_factor * 2), 2.0)
        ...
        self.cls_head = CLSHead(embed_dim, head_dropout=dropout)
        self.output_norm = nn.LayerNorm(embed_dim)

    def _build_patch_mask(self, traffic_mask: torch.Tensor, padding: int) -> torch.Tensor:
        mask = traffic_mask.float()
        if padding > 0:
            mask = F.pad(mask, (0, padding))
        patch_mask = mask.unfold(dimension=-1, size=self.patch_len, step=self.stride).amax(dim=-1)
        return patch_mask.bool()

    def _tokenize(self, traffic_feats: torch.Tensor, traffic_mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, int]:
        x = self.input_proj(self.input_norm(traffic_feats))
        x = x.masked_fill(~traffic_mask.unsqueeze(-1), 0.0)
        x = x.permute(0, 2, 1)
        remainder = x.shape[-1] % self.patch_len
        padding = 0 if remainder == 0 else self.patch_len - remainder
        if padding > 0:
            x = F.pad(x, (0, padding))
        x, n_vars = self.patch_embeddings(x)
        x = x.view(-1, n_vars, x.shape[-2], x.shape[-1])
        patch_mask = self._build_patch_mask(traffic_mask, padding)
        x = x + self.position_embedding(x)
        x = x * patch_mask[:, None, :, None].float()
        return x, patch_mask, padding
    ...
    def forward(
        ...
        tokens, patch_mask, _ = self._tokenize(traffic_feats, traffic_mask)
        batch_size = tokens.shape[0]
        cls_tokens = self.cls_tokens.expand(batch_size, -1, -1, -1)
        x = torch.cat([tokens, cls_tokens], dim=2)
        seq_token_len = tokens.shape[2]

        for block in self.blocks:
            x = block(x, prefix_seq_len=seq_token_len, attn_mask=None)

        x = self.output_norm(x)
        x[:, :, :seq_token_len] = x[:, :, :seq_token_len] * patch_mask[:, None, :, None].float()
        cls_feature = self.cls_head(x, return_feature=True).squeeze(dim=2)
        global_repr = self.output_norm(cls_feature.mean(dim=1))
        traffic_sequence = self.output_norm(x[:, :, :seq_token_len, :].mean(dim=1))
        traffic_sequence = traffic_sequence * patch_mask.unsqueeze(-1).float()
