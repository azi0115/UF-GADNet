"""Traffic-transformer-only training entrypoint."""
...
from models import TrafficTransformerOnlyDetector
...
def main() -> None:
    config = get_config()
    setup_logging(config.log_dir, "train_traffic_transformer_only")
    set_seed(config.seed)
    device = get_device(config.device)

    train_records = load_records(config.train_path)
    val_records = load_records(config.val_path)
    logger.info("Training mode: traffic_transformer_only")
    logger.info(
        "traffic-transformer defaults | traffic_input_dim=%d | traffic_embed_dim=%d | traffic_layers=%d | traffic_heads=%d | max_traffic_len=%d | batch_size=%d | lr=%.2e | weight_decay=%.2e | dropout=%.2f",
        config.traffic_input_dim,
    ...
    # Reuse the existing dataloader interface; the model ignores URL tensors.
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if val_metrics["auc"] > best_auc:
            ...
            patience_counter = 0
            save_checkpoint(
                path=os.path.join(config.checkpoint_dir, "best_model_traffic_transformer_only.pt"),
                model=model,
                optimizer=optimizer,
                config=config,
                metrics=val_metrics,
        ...
        if config.save_last_checkpoint:
            save_checkpoint(
                path=os.path.join(config.checkpoint_dir, "last_model_traffic_transformer_only.pt"),
                model=model,
                optimizer=optimizer,
                config=config,
                metrics=val_metrics,
    ...
    logger.info("traffic_transformer_only best_auc=%.4f best_epoch=%d", best_auc, best_epoch)
